...
func TestSHA256ValidityRoute(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/internal/sha256/validity", bytes.NewReader(body))
...
func TestSHA256ValidityRouteValidation(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/internal/v1/sha256/validity", bytes.NewBufferString(`{"sha256":["   "]}`))
	rr := httptest.NewRecorder()
	router.ServeHTTP(rr, req)

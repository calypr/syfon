...
func TestMetricsRoutes_ListAndSummary(t *testing.T) {
	...
	t.Run("list", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodGet, "/internal/metrics/v1/files?limit=10&offset=0&inactive_days=365", nil)
	...
	t.Run("summary", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodGet, "/internal/metrics/v1/summary?inactive_days=365", nil)
		rr := httptest.NewRecorder()
		router.ServeHTTP(rr, req)
		if rr.Code != http.StatusOK {
			t.Fatalf("expected 200, got %d body=%s", rr.Code, rr.Body.String())
...
func TestMetricsRoutes_GetNotFoundAndValidation(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/internal/metrics/v1/files/missing", nil)
	rr := httptest.NewRecorder()
	router.ServeHTTP(rr, req)
	...
	req2 := httptest.NewRequest(http.MethodGet, "/internal/metrics/v1/files?limit=0", nil)
...
func TestMetricsSummaryAuthzAndScope(t *testing.T) {
	...
	t.Run("scope reader can access scoped summary", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodGet, "/internal/metrics/v1/summary?organization=cbds&project=end_to_end_test", nil)
	...
	t.Run("indexd_admin style reader can access global summary via /programs read", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodGet, "/internal/metrics/v1/summary", nil)
		ctx := context.WithValue(req.Context(), core.AuthModeKey, "gen3")
		ctx = context.WithValue(ctx, core.AuthHeaderPresentKey, true)
		ctx = context.WithValue(ctx, core.UserPrivilegesKey, map[string]map[string]bool{
			"/programs": {"read": true},
...
func TestMetricsFilesAuthzAndScope(t *testing.T) {
	...
	t.Run("scoped list returns only scoped objects", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodGet, "/internal/metrics/v1/files?organization=cbds&project=end_to_end_test&limit=10&offset=0", nil)
		ctx := context.WithValue(req.Context(), core.AuthModeKey, "gen3")
		ctx = context.WithValue(ctx, core.AuthHeaderPresentKey, true)
		ctx = context.WithValue(ctx, core.UserPrivilegesKey, map[string]map[string]bool{
			"/programs/cbds/projects/end_to_end_test": {"read": true},
	...
	t.Run("scoped object lookup outside scope returns 404", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodGet, "/internal/metrics/v1/files/other-1?organization=cbds&project=end_to_end_test", nil)
		ctx := context.WithValue(req.Context(), core.AuthModeKey, "gen3")
		ctx = context.WithValue(ctx, core.AuthHeaderPresentKey, true)
		ctx = context.WithValue(ctx, core.UserPrivilegesKey, map[string]map[string]bool{
			"/programs/cbds/projects/end_to_end_test": {"read": true},
	...
	t.Run("global object lookup allowed via /programs read", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodGet, "/internal/metrics/v1/files/other-1", nil)
		ctx := context.WithValue(req.Context(), core.AuthModeKey, "gen3")
		ctx = context.WithValue(ctx, core.AuthHeaderPresentKey, true)
		ctx = context.WithValue(ctx, core.UserPrivilegesKey, map[string]map[string]bool{
			"/programs": {"read": true},
